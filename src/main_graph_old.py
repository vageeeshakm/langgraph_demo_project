...
def router_agent(state):
    ...
    router_agent_ex = create_react_agent(
        model, tools=[], prompt=system_message
    )
...
def api_assistent(state):
    ...
    api_agent_ex = create_react_agent(
        api_model, api_tools, prompt=system_message
    )
    
    for each in state['messages']:
        if isinstance(each, HumanMessage):
            input_with_history.append(
                ("user", each.content)
            )

    api_result = api_agent_ex.invoke({
            'messages': input_with_history
        },
        config
    )
...
while True:
    ...
    print("Done!")
    # # 💬 Show all messages
    # for msg in result["history"][-2:]:  # only show last turn
